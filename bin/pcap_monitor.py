...
class PcapMonitorModularInput(ModularInput):
	...
	def _shellquote(self, s):
		"""
		Escape the string used in shell to avoid security risks
		"""
	...
	def parseNewPcap(self, stz, params, config): 
		...
		for pcap_file in pcapToParse:
			...
			if( os.path.isfile(bro_seeds) ):
				bro_seeds_cmd = self._shellquote(bro_seeds)
				cmd =  "export BRO_SEED_FILE=%s && " % bro_seeds_cmd

			if( (bro_opts == None) or (bro_opts == "None") ):
				bro_opts_cmd = ""
			else:
				bro_opts_cmd = self._shellquote(bro_opts)

			bro_bin_cmd = self._shellquote(bro_bin)
			pcap_file_cmd = self._shellquote(pcap_file)
			cmd += "%s %s -r %s" % (bro_bin_cmd, bro_opts_cmd, pcap_file_cmd)

			if( os.path.isfile(bro_script) ):
				bro_script_cmd = self._shellquote(bro_script)
				cmd += " %s " % bro_script_cmd
...
if __name__ == '__main__':
	try:
		if os.name == 'nt':
			logger.error("Bro IDS TA doesn't support Windows platform yet. Please try it on Unix")
			sys.exit()
